...
<div class="panel-body panel-white">
 <ul class="nav nav-tabs panel_tabs" role="tablist">
      <li class="<?php if($active_tab=='stud_attendance'){?>active<?php }?>">
          <a href="?dashboard=user&page=view-attendance&tab=stud_attendance&student_id=<?php echo $student_id;?>">
             <i class="fa fa-align-justify"></i> <?php _e('Attendance', 'school-mgt'); ?></a>
          </a>
      </li>
     <li class="<?php if($active_tab=='sub_attendance'){?>active<?php }?>">
			<a href="?dashboard=user&page=view-attendance&tab=sub_attendance&student_id=<?php echo $student_id;?>" class="tab <?php echo $active_tab == 'sub_attendance' ? 'active' : ''; ?>">
				...
				<input type="hidden" name="user_id" value=<?php echo $_REQUEST['student_id'];?>>       
					<div class="form-group col-md-3">
						<label for="exam_id"><?php _e('Strat Date','school-mgt');?></label>
								...
								<input type="text"  class="form-control sdate" name="sdate" value="<?php if(isset($_REQUEST['sdate'])) echo smgt_getdate_in_input_box($_REQUEST['sdate']);else echo date('Y-m-d');?>">
								
					</div>
					<div class="form-group col-md-3">
						<label for="exam_id"><?php _e('End Date','school-mgt');?></label>
							<input type="text"   class="form-control edate" name="edate" value="<?php if(isset($_REQUEST['edate'])) echo smgt_getdate_in_input_box($_REQUEST['edate']);else echo date('Y-m-d');?>">
