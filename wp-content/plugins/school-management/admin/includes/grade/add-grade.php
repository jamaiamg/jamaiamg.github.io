...
$(document).ready(function() {
	 $('#grade_form').validationEngine();
        ...
        <div class="form-group">
			...
			<div class="col-sm-8">
				<input id="grade_name" class="form-control validate[required]" type="text" value="<?php if($edit){ echo $grade_data->grade_name;}?>" name="grade_name">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="grade_point"><?php _e('Grade Point','school-mgt');?><span class="require-field">*</span></label>
			<div class="col-sm-8">
				<input id="grade_point" class="form-control validate[required,custom[onlyNumberSp]] text-input" type="number" value="<?php if($edit){ echo $grade_data->grade_point;}?>" name="grade_point" style="width:60px">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="mark_from "><?php _e('Mark From','school-mgt');?><span class="require-field">*</span></label>
			<div class="col-sm-8">
				<input id="mark_from" class="form-control validate[required,custom[onlyNumberSp]] text-input" type="number" value="<?php if($edit){ echo $grade_data->mark_from;}?>" name="mark_from" style="width:60px">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="mark_upto"><?php _e('Mark Upto','school-mgt');?><span class="require-field">*</span></label>
			<div class="col-sm-8">
				<input id="mark_upto" class="form-control validate[required,custom[onlyNumberSp]] text-input" type="number" value="<?php if($edit){ echo $grade_data->mark_upto;}?>" name="mark_upto" style="width:60px">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="grade_comment"><?php _e('Comment','school-mgt');?></label>
			<div class="col-sm-8">
			<textarea name="grade_comment" class="form-control" id="grade_comment"><?php if($edit){ echo $grade_data->grade_comment;}?></textarea>
